...
# 加载完整图结果
df = pd.read_csv('logs/arxiv_continued.csv')
...
ax.set_title('Sparsity Robustness on Full ogbn-arxiv (169k nodes)', fontsize=16)
ax.legend(loc='lower left', fontsize=12)
ax.set_xticks([0, 25, 50, 75, 90])
ax.set_ylim([50, 66])
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'full_sparsity_robustness.png'), dpi=150)
print("Saved: full_sparsity_robustness.png")
plt.close()
...
ax.set_xlabel('Edge Drop Rate', fontsize=14)
ax.set_ylabel('Test Accuracy (%)', fontsize=14)
ax.set_title('Model Comparison on Full ogbn-arxiv', fontsize=16)
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'full_model_comparison.png'), dpi=150)
...
ax.set_title('LLM Embedding Improvement over Raw Features', fontsize=16)
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'full_llm_improvement.png'), dpi=150)
print("Saved: full_llm_improvement.png")
plt.close()
...
ax.set_xlabel('Edge Drop Rate', fontsize=14)
ax.set_ylabel('Accuracy Improvement (%)', fontsize=14)
ax.set_title('LLM-GNN Improvement over Baselines', fontsize=16)
ax.set_xticks(x)
ax.set_xticklabels([f'{int(r*100)}%' for r in drop_rates])
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'full_ours_improvement.png'), dpi=150)
print("Saved: full_ours_improvement.png")
plt.close()
...
ax.set_title('Test Accuracy (%) on Full ogbn-arxiv (169k nodes)', fontsize=16, pad=20)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'full_results_table.png'), dpi=150)
